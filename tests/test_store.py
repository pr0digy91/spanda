...
def test_indexing_unchanged_code_twice_produces_zero_drift(workspace):
    ...
    assert rows["uuids"] == rows["keys"] == 53
...
def test_unchanged_files_do_not_write_a_row_every_scan(tmp_path):
    ...
    assert files == 16, "one row per file, not per file per scan"
    assert versions == 16, "every file recorded once, at the scan that found it"
